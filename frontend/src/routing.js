...
export function initRouter() {  // инициализация маршрутизатора приложения
  function initAutorization() { // функция для обработки формы ввода
    u('.main').html('');
    u('.main').append(RenderElem.loginForm());
    const inputPassword = document.querySelector('.password');
    const inputLogin = document.querySelector('.login');
    inputLogin.addEventListener('input', (e) => {  //устанавливаем обработчики событий
    ...
    u('.main__login-from').on('submit', async (e) => {
      ...
      const password = inputPassword.value;  //получаем логин пароль
      const login = inputLogin.value;
      let invalid;
      if (!lengthValidation(login, 6)) {  // проверяем длинну
        RenderElem.error(
          u('.login-wrapper'),
      ...
      if (login.length >= 6 && login.toLowerCase() !== 'developer') {
        RenderElem.error(
            u('.login-wrapper'),
      ...
      if (invalid) {  // Если есть ошибки - выходим
        u('.spinner').remove();
        return;
      }
      await BankingApi.pushlogin(login, password) // Вызываем функцию pushlogin для авторизации
        .then(async (res) => {
          localStorage.setItem('token', res.payload.token);
          router.navigate('account'); // Переходим на страницу учетной записи
        })
        .catch((err) => {
          if (err.type === 'login') {  // проверяем ошибки
        ...
        .finally(() => {
          u('.spinner').remove(); //удаляем спинер
        });
    });
  }

  // Функция для проверки наличия токена учетной записи
  ...
  function exitApp() {
    localStorage.removeItem('token'); //удаляем токен из локал сторадж при выходе
  ...
  async function makeTranzaction(e, CurrentPage, thisAccauntNumber) { // валидация транзакции
  ...
  async function chargeCurence(from, to, token, input, CurrentPage) {  // валидация обмена валют
    ...
    await BankingApi.exchangeCurrency(from, to, amount, token)
      .then((res) => {
        CurrentPage.updateAccounts(res.payload);
  ...
  router.on('/', () => {
    const token = localStorage.getItem('token'); // Получаем токен учетной записи из localStorage
    if (accountToken(token)) return;
    router.navigate('account');
  });

  router.on('/account', async () => { // Устанавливаем обработчик событий для страницы счетов
    const token = localStorage.getItem('token'); // Получаем токен учетной записи из localStorage
    if (accountToken(token)) return;
    u('title').text('Счета');
    u('main').html('');
  ...
  router.on('account/details/:id', async ({ data }) => { //Устанавливаем обработчик событий для страницы деталей счета
    const token = localStorage.getItem('token');
    if (accountToken(token)) return;
    u('main').html('');
  ...
  router.on('account/details/history/:id', async ({ data }) => { // Устанавливаем обработчик событий для страницы истории операций
    const token = localStorage.getItem('token');
    if (accountToken(token)) return;
    u('main').html('');
    RenderElem.initNav(router);
  ...
  router.on('/banks', async () => {  // Устанавливаем обработчик событий для страницы с отделениями банков
    const token = localStorage.getItem('token');
    if (accountToken(token)) return;
    u('main').html('');
  ...
  router.on('/currency', async () => {  // Устанавливаем обработчик событий для страницы с валютами
    const token = localStorage.getItem('token');
    if (accountToken(token)) return;
    u('main').html('');
