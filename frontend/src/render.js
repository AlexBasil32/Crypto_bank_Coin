...
export class RenderElem { 
  static loginForm() { // рендерим форму логин/пароль при помощи umbrellajs
    return u('<form>')
...
// очищаем все ошибки перед валидацией формы
  ...
  static spinner(elem) {  //добавляем спинер загрузки
    elem.append(u('<div>').addClass('spinner'));
  }

  static navMenu(elem) { // добавляем навигационное меню
    const nav = u('<div>')
  ...
  static initDropdownInput(value, dropdownMenu, list, input) { // здесь описана логика выпадающего списка
    const valuesList = [];
  ...
  static initNav(router, path) {  //инициализируем навигационное меню в шапке сайта
  ...
  static activateNavMenu(elem) {   //активируем пункты навигационного меню
    elem.children().each((el) => {
      el.removeAttribute('disabled');
    });
  }

  static renderUlSocketList(data, elem) {  //выводим данные на страницу, обнавляем курсы валют в режиме реального времени
...
class Page {
  ...
  parseMethods(btnActionObj) {   //метод parseMethods - принимает объект с парой ключ-значение
    for (let btn of btnActionObj) { // проходится по каждому элементу объекта btnActionObj
      u(btn[0]).on(btn[1][1], btn[1][0]); // устанавливаем обработчик нажатия на кнопку
    }
  }
}

export class HeadPage extends Page { // в этом классе мы создаем дропдаун с фильтрацией
  render(elem, accs) {
    ...
    container
      ...
      .append(
        u('<button>').addClass('account__button').text('Создать новый счёт')  // добавляем кнопку создать счет
      );
    const gridAccs = u('<ul>').addClass('account__list'); // перебираем каждый аккаунт в массиве
    for (let a of accs) {
      let balance = String(a.balance);  // запись баланса в виде строки
      balance =
        balance.replace(/(\d{1,3}(?=(?:\d\d\d)+(?!\d)))/g, '$1' + ' ') + ' ₽'; // форматируем строку баланса с помощью регулярного выражения
      let lastDateTrans;
      if (a.transactions.length > 0) {  // данные последней транзакции
  ...
  updateList(accs, func) {  //обновляем список счетов на странице, очищая его содержимое и заменяя его на новый список счетов.
...
export class DetailsPage extends Page { // информация детализации счета
  cleanData(transactions, balanceCurrent, lastTrans, countStat, ourFrom) {
    ...
    return [dataKeys.reverse(), dataVal.reverse()]; //возвращает данные в формате, который удобен для использования в таблице с датами и балансом на эти даты. 
                                                    //Он добавляет данные только для тех дат, на которые были транзакции в исходном массиве, игнорируя месяцы без транзакций.
  }
  ...
  cleanDataStructor(transactions, ourFrom, lastTrans, countStat) {
    ...
    return [labels.reverse(), dataValPlus.reverse(), dataValMinus.reverse()]; //возвращает данные для графика, показывая количество как положительных, так и отрицательных транзакций на каждый месяц. 
  }

  renderBalance(elem, accs) {  // рендерим баланс
  ...
  updateBalance(res) { // обновляем баланс
    let data = res.toString();
    data = data.replace(/(\d{1,3}(?=(?:\d\d\d)+(?!\d)))/g, '$1' + ' ') + ' ₽';
    u('.amount-balance').text(data);
  }

  appendInTable(accs) {  // добавляем строки в таблицу со списком транзакций
  ...
  renderTable(elem, accs, countRow) { // отрисовка таблицы со списком транзакций и графика динамики баланса.
    const ourFrom = accs.account;
    ...
    if (accs.transactions.length === 0) {  //Если в объекте accs нет транзакций, добавляется элемент-заглушка с сообщением о том, что история транзакций отсутствует, и функция завершается.
  ...
  renderStdGrafic(elem, accs, countCol, className) {
    ...
    // Значения максимального и минимального баланса используются для вычисления правильного диапазона значений оси Y на графике.
    ...
    const chartAreaBorder = {  // новый элемент управления для графика, который рисует рамку вокруг области графика.
      id: 'chartAreaBorder',
    ...
    const options = {  //опции графика
      responsive: true,  // график должен быть адаптивным 
      maintainAspectRatio: false, //соотношение сторон графика не сохраняется при изменении размеров;
      scales: { //настройки осей x и y;
        x: {
          grid: {
            color: 'rgba(0, 0, 0, 0)', 
          },
      ...
      plugins: {
        legend: {  // отображаем легенду графика
          display: false,
        },
        chartAreaBorder: { // плагин для отображения толщины рамки вокруг графика
          borderColor: 'black',
          borderWidth: 1,
        },
      },
    ...
    const config = {  // конфиг для графика
      type: 'bar', 
      data: data,
      options: options,
    ...
    new Chart(document.getElementById('plot'), config); //добавляем созданый обьект графика на страницу
  }

  renderStructorGrafic(elem, accs, countCol, className) {  // рендерим структуру графика
    elem.append(
      u('<div>')
        .addClass(className, 'to-history')
        .append(
  ...
  render(elem, accs) {
    const container = u('<div>').addClass('container', 'main__container');  // рендерим структуру формы счета
    elem.append(container);
    container
      .append(
        u('<h1>')
  ...
  init(btnActionObj) {   //отрисовываем различные элементы
    this.render(this.elemPaste, this.data);
    this.parseMethods(btnActionObj);
    return;
  }
...
export class HistoryPage extends DetailsPage {  // отрисовываем историю переводов
  render(elem, accs) {
    const container = u('<div>').addClass('container', 'main__container');
    elem.append(container);
    container
...
export class BanksPage extends Page {  // страница с адресами банкоматов на Яндекс карте
...
export class CurrencyPage extends Page { // отображаем страницу валютного обмена с наследованием от класса Page
  renderDropdown(list, value) {  // список валют и текущее значение, возвращает выпадающее меню с этими валютами. 
    const menu = u('<ul>')
      .addClass('dropdown-menu')
  ...
  renderItemsAccs(currencyAccount, currencyListElem) {  // отображения списка валют и баланса на каждом счете.
  ...
  render(elem, accs) {  // отображение основной разметки
  ...
  updateAccounts(accs) {  // обновления списка валют и баланса после каждой операции обмена.
    u('.currency-list').html('');
    this.renderItemsAccs(accs, u('.currency-list'));
  }

  init(btnActionObj) {  // инициализирум и связываем элементы
    this.render(this.elemPaste, this.data);
    this.parseMethods(btnActionObj);
    return;
  }
